...
def parse_parser_xml_results(xml, file_name="", raw_output=False):
    ...
    def extract_words_from_xml(sent_node):
        if type(sent_node['tokens']['token']) == type(OrderedDict()):
            ...
            exted = sent_node['tokens']['token']
        exted_string = [x['word'] for x in exted]
        return exted_string
    ...
    document = raw_dict['root']['document']

    # Making a raw sentence list of dictionaries:
    raw_sent_list = document['sentences']['sentence']

    if document.get('coreference') and document['coreference'].get('coreference'):
        # Convert coreferences to the format like python
        coref_flag = True

        # Making a raw coref dictionary:
        raw_coref_list = document['coreference']['coreference']
        ...
        coref_index = [[[int(raw_coref_list[j]['mention'][i]['sentence']) - 1,
                        ...
                        for i in range(len(raw_coref_list[j]['mention']))]
                       for j in range(len(raw_coref_list))]

        coref_list = []
        for j in range(len(coref_index)):
            ...
            for k, coref in enumerate(coref_index[j]):
                ...
                exted_words = [x['word'] for x in exted]
                coref_list[j][k].insert(0, ' '.join(exted_words))

        coref_list = [[[coref_list[j][i], coref_list[j][0]]
                       for i in range(len(coref_list[j])) if i != 0]
                      for j in range(len(coref_list))]
    else:
        coref_flag = False
    ...
    sentences = []
    for id in range(len(raw_sent_list)):
        sent = {}
        ...
        sent['dependencies'] = [[enforceList(dep['dep'])[i]['@type'],
                                ...
                                for i in range(len(enforceList(dep['dep'])))
